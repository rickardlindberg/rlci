...
class ZeroApp:
    ...
    PROCESS => ['ssh', 'rlci@ci.rickardlindberg.me', 'python', '/opt/rlci/rlci-cli.py', 'trigger', 'rlci']
    ...
    ...                     '/opt/rlci/rlci-cli.py', 'trigger', 'rlci'],
    ...         "returncode": 99,
    ...     }
    ... ]).filter("PROCESS", "EXIT")
    PROCESS => ['git', 'checkout', '-b', 'BRANCH']
    ...
    def run(self):
        if self.args.get() == ["build"]:
            ...
            try:
                self.process.run(["ssh", "rlci@ci.rickardlindberg.me", "python", "/opt/rlci/rlci-cli.py", "trigger", "rlci"])
            finally:
